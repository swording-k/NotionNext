...
公众号、小红书、知乎、抖音负责被看见和互动；NotionNext 博客负责长期沉淀代表作、作品集和个人品牌背书。
...
3. 文末加入合规入口：公众号默认加个人网站和博客文字链接；不默认加二维码。
4. 保存到微信公众号草稿箱。
5. 给出小红书、抖音、知乎可用的标题/摘要/改写方向。
6. 如果用户要求博客发布，再同步 Notion 和部署 Vercel。
7. 提交本地变更并推送 `fork main`。
8. 最终答复必须说明：文件路径、公众号草稿是否成功、博客是否成功部署、是否有人工步骤。
...
- 知乎：可加参考链接或个人主页，但回答必须站内完整可读。
- 小红书：不要在正文、图片、封面里默认放站外网址、二维码、联系方式、水印式导流。
...
6. 重新部署 Vercel，因为当前站点是 `EXPORT=true` 静态导出。
7. 验证公开 URL：
